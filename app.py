...
st.markdown("""
    <style>
    .stApp { background-color: #4B3832; color: white; } /* تعديل لون الخلفية ليتماشى مع صورتك */
    ...
    .stTextInput label { color: #85a5a1 !important; font-size: 16px !important; font-weight: normal !important; direction: rtl !important; text-align: right !important; display: block;}
    .stTextInput > div > div > input { background-color: #5d4b45; color: white; border: none; font-size: 18px; direction: ltr !important; border-radius: 8px;}
    
    /* ---------------------------------------------------
       تصميم لوحة المفاتيح الاحترافية المشابهة لـ GeoGebra
       --------------------------------------------------- */
    .keyboard-wrapper {
    ...
    /* تخصيص ألوان الأزرار حسب الوظيفة */
    
    /* المتغيرات (x, y, m, pi) */
    .btn-var > button { background-color: #5d4b45 !important; color: white !important; font-style: italic;}
    
    /* الأرقام */
    .btn-num > button { background-color: #6a5750 !important; color: white !important; font-weight: bold !important;}
    
    /* العمليات (/, *, -, +, <, >, =) */
    .btn-op > button { background-color: #5d4b45 !important; color: white !important; }
    
    /* الدوال (ln, sqrt, abs) */
    .btn-func > button { background-color: #5d4b45 !important; color: white !important; font-size: 16px !important;}
    
    /* الأقواس */
    .btn-paren > button { background-color: #5d4b45 !important; color: white !important; }
    
    /* أزرار الحذف والمسح */
    .btn-del > button { background-color: #5d4b45 !important; color: #ff6b6b !important; }
    .btn-clear > button { background-color: #3e2e2a !important; color: #ff6b6b !important; height: 45px !important; border-radius: 8px !important; margin-top: 10px !important;}
    
    /* تأثير الضغط */
    div[data-testid="stExpander"] div[data-testid="stButton"] > button:active {
        background-color: #85a5a1 !important;
    ...
    div[data-testid="stExpander"] summary { display: none !important; } /* سنبقيها مفتوحة دائما كما في الصورة */
...
if 'g_val' not in st.session_state: st.session_state.g_val = ""
if 'kbd_target' not in st.session_state: st.session_state.kbd_target = "الدالة f(x)"

# توجيه الكتابة (راديو باتون)
st.markdown("<div class='radio-container'>توجيه الكتابة إلى:</div>", unsafe_allow_html=True)
col_r1, col_r2 = st.columns([1, 1])
with col_r2:
    target_f = st.checkbox("الدالة f(x)", value=st.session_state.kbd_target == "الدالة f(x)")
with col_r1:
    target_m = st.checkbox("المستقيم بدلالة m", value=st.session_state.kbd_target == "المستقيم بدلالة m")

if target_f and not target_m: st.session_state.kbd_target = "الدالة f(x)"
elif target_m and not target_f: st.session_state.kbd_target = "المستقيم بدلالة m"
...
# بناء لوحة المفاتيح بشكل دائم (غير مخفية بـ expander)
st.markdown("<div class='keyboard-wrapper'>", unsafe_allow_html=True)

# مصفوفة الأزرار مع تحديد فئة CSS لكل زر
rows = [
    [("𝑥", "x", "btn-var"), ("𝑦", "y", "btn-var"), ("𝜋", "pi", "btn-var"), ("7", "7", "btn-num"), ("8", "8", "btn-num"), ("9", "9", "btn-num")],
    [("𝑚", "m", "btn-var"), ("𝑒", "e", "btn-var"), ("ln", "ln(", "btn-func"), ("4", "4", "btn-num"), ("5", "5", "btn-num"), ("6", "6", "btn-num")],
    [("□²", "^2", "btn-func"), ("√", "sqrt(", "btn-func"), ("|□|", "abs(", "btn-func"), ("1", "1", "btn-num"), ("2", "2", "btn-num"), ("3", "3", "btn-num")],
    [("<", "<", "btn-op"), (">", ">", "btn-op"), ("=", "=", "btn-op"), ("0", "0", "btn-num"), (".", ".", "btn-num"), ("⌫", "⌫", "btn-del")],
    [("(", "(", "btn-paren"), (")", ")", "btn-paren"), ("+", "+", "btn-op"), ("-", "-", "btn-op"), ("*", "*", "btn-op"), ("/", "/", "btn-op")]
]
...
with st.container():
    for r_idx, row in enumerate(rows):
        cols = st.columns(6)
        for c_idx, (label, val, cls_name) in enumerate(row):
            # تطبيق الكلاس عبر الحاوية
            with cols[c_idx]:
                st.markdown(f'<div class="{cls_name}">', unsafe_allow_html=True)
                st.button(label, on_click=k_click, args=(val,), key=f"btn_{r_idx}_{c_idx}")
